...
netlist = sdn.parse("pong.v",architecture=XILINX_7SERIES,remove_space=True)
...
hinstances_to_replicate = list(
    netlist.get_hinstances(
        recursive=True, filter=lambda x: x.item.reference.is_leaf() is True
                                                    ...
                                                    and "obuf" not in x.name.lower()
                                                    and "OBUF" not in x.item.reference.name
                                                    and "PLLE2" not in x.item.reference.name
                                                    and "BUFG" not in x.item.reference.name))
...
hports_to_replicate = list(netlist.get_hports(filter = lambda x: x.item.direction is sdn.OUT
                                              and "VGA" not in x.name 
                                              and "segment" not in x.name 
                                              and "anode" not in x.name))
...
netlist.compose("pong_tmr.v", voters, reinsert_space=True)
