...
from spydrnet_tmr.utils.design_rule_check.drc_insert_organs import (
    check_organs,
    check_if_organ_was_inserted,
    check_if_organ_inputs_from_each_domain,
    check_if_organ_outputs_to_own_domain,
)
import unittest
...
class TestDRCCInsertOrgans(unittest.TestCase):
    ...
    def test_drc_pass(self):
        self.voters = insert_organs(
            self.replicas, self.insertion_points, XilinxTMRVoter(), "VOTER"
        )
        self.assertTrue(
            check_organs(self.insertion_points, self.voters, ["VOTER"], "TMR")
        )

    def test_drc_fail(self):
        # insert at all but one spot
        self.voters = insert_organs(
            self.replicas, self.insertion_points[1:], XilinxTMRVoter(), "VOTER"
        )
        self.assertFalse(
            check_if_organ_was_inserted(self.insertion_points, ["VOTER"])
        )
        ...
        voter_1_neighbors = list(
            x
            for x in voter_1_output.wire.get_pins(selection=Selection.OUTSIDE)
            if x is not voter_1_output
        )
        ...
        self.assertFalse(
            check_if_organ_outputs_to_own_domain(self.voters, "TMR")
        )
        ...
        self.assertFalse(
            check_if_organ_inputs_from_each_domain(self.voters, "TMR")
        )
