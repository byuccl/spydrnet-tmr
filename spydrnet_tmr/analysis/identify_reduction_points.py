...
def identify_reduction_points(replicas, suffix):
    ...
    associated_netlist = next(
        list(netlist for netlist in replicas[x][0].get_netlists())[0]
        for x in replicas.keys()
    )
    global TOP_INSTANCE
...
def get_and_compare_next_instances(primary, other_pins, suffix):
    ...
    return compare_next_instances(
        primary, next_instances_original, next_instances_replicated, suffix
    )
...
def compare_next_instances(
    current_output_pin, next_instances_original, next_instances_replicated, suffix
):
    reduction_points = []
